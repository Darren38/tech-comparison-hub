...
function currencySelect(variant) {
  ...
  return html`<label class="currency currency--${variant}" title="Display currency">
    <span class="${variant === 'mobile' ? 'currency__label small' : 'sr-only'}">Currency</span>
    <select data-currency aria-label="Display currency">${currencyList().map((c) => html`<option value="${c.code}" ${c.code === current ? 'selected' : ''}>${c.label}</option>`)}</select>
  </label>`;
}
  ...
  mount(root, html`
    <div class="wrap site-header__inner">
      <a class="brand" href="#/" aria-label="Tech Comparison Hub home">
        ...
        <span class="brand__text">Tech Comparison Hub<small>Evidence-first research</small></span>
      </a>
      <nav class="nav" aria-label="Main">${NAV.map((n) => html`<a href="${href(n.path)}" data-nav="${n.match.join(' ')}">${n.label}</a>`)}</nav>
      <div class="header-tools">
        <form class="header-search searchbox" role="search" data-searchbox>
          ${icon('search', { size: 16 })}
          <input type="search" name="q" placeholder="Search devices, chips, news…" autocomplete="off" aria-label="Search" />
        </form>
        <a class="icon-btn search-btn" href="${href('/search')}" aria-label="Search">${icon('search')}</a>
        ${currencySelect('header')}
        <button type="button" class="icon-btn" data-action="theme" aria-label="Toggle colour theme">${icon(theme.effective() === 'dark' ? 'sun' : 'moon')}</button>
        <button type="button" class="icon-btn menu-btn" data-action="menu" aria-label="Open menu" aria-expanded="false" aria-controls="mobile-nav">${icon('menu')}</button>
      </div>
    </div>
    <nav class="mobile-nav" id="mobile-nav" aria-label="Main (mobile)">${NAV.map((n) => html`<a href="${href(n.path)}" data-nav="${n.match.join(' ')}">${n.label}</a>`)}${currencySelect('mobile')}</nav>`);
  ...
  const update = async () => {
    ...
    mount(list, html`
      ...
      ${groups.length
        ...
        : intentTarget ? '' : html`<div class="suggest__group small muted">No matches. Press Enter to search everything.</div>`}
      <a class="suggest__item tiny muted" role="option" id="${listId}-${n++}" href="${href('/search', { q })}">See all results for “${q}”</a>`);
...
export function renderTray(path) {
  ...
  mount(tray, html`
    <span class="tray__label">Compare ${ids.length}/${MAX_COMPARE}</span>
    <div class="tray__items">${ids.map((id) => html`<span class="tray__item">${store.deviceById.get(id).name}<button type="button" data-tray-remove="${id}" aria-label="Remove ${deviceTitle(store.deviceById.get(id))} from comparison">${icon('close', { size: 14 })}</button></span>`)}</div>
    <button type="button" class="btn btn--sm btn--ghost tray__clear" data-tray-clear>Clear</button>
    <a class="btn btn--sm btn--accent" href="${href(`/compare/${ids.join(',')}`)}" ${ids.length < 2 ? html`aria-disabled="true"` : ''}>${ids.length < 2 ? 'Add one more' : 'Compare now'} ${icon('arrow', { size: 14 })}</a>`);
}
...
export function renderFooter() {
  ...
  mount(document.getElementById('site-footer'), html`
    <div class="wrap site-footer__grid">
      <div>
        <div class="brand__text" style="color:var(--ink)">Tech Comparison Hub</div>
        <p style="margin-top:8px;max-width:48ch">An evidence-first research platform. Facts are attributed to their sources and linked; reviews are summarised in our own words, never republished. Platform scores are labelled as analysis.</p>
        <p class="tiny" style="margin-top:10px"><strong>Data as of ${fmtDate(data.asOf ?? time.slice(0, 10))}, not live.</strong> Devices announced ${fmtDate(data.announcedFrom, 'month')} to ${fmtDate(data.announcedTo, 'month')} · ${fmtNumber(counts.devices)} devices · ${fmtNumber(counts.records)} evidence records · ${fmtNumber(counts.documents)} source documents · <a href="${href('/methodology', { section: 'freshness' })}">How current is this?</a></p>
        <p class="tiny muted" style="margin-top:6px" data-rates-label>Exchange rates: ${ratesLabel()}${store.core.currencies?.live ? ' · updated when you opened this page' : ''}.</p>
      </div>
      <div><h4>Explore</h4><ul>
        <li><a href="${href('/devices/smartphone')}">Smartphones</a></li>
        <li><a href="${href('/devices/tablet')}">Tablets</a></li>
        <li><a href="${href('/devices/smartwatch')}">Smartwatches</a></li>
        <li><a href="${href('/chipsets')}">Chipsets</a></li></ul></div>
      <div><h4>Evidence</h4><ul>
        <li><a href="${href('/reviews')}">Reviews & videos</a></li>
        <li><a href="${href('/news')}">News</a></li>
        <li><a href="${href('/coverage')}">Evidence coverage</a></li>
        <li><a href="${href('/methodology', { section: 'sources' })}">Source registry</a></li></ul></div>
      <div><h4>About</h4><ul>
        <li><a href="${href('/methodology')}">How scoring works</a></li>
        <li><a href="${href('/methodology', { section: 'confidence' })}">Confidence levels</a></li>
        <li><a href="${href('/methodology', { section: 'ethics' })}">Data ethics</a></li></ul></div>
    </div>`);
}
