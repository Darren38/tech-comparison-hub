        ...
        <section id="freshness">
          ...
          <div class="card small stack">
            ...
            <ul class="rules">
              ...
              <li><strong>New phones join the charts by themselves.</strong> A phone nobody has matched by hand is added to the 3DMark, DXOMARK and AnTuTu results when a listing has its exact model name and, where the source names the chip, the same chip the hub records for it; its UL device page is then read like any other. DXOMARK names no chip, so there the full name must match, including "5G" (a 4G and a 5G version are often different phones). Geekbench 6 averages come from each phone's NanoReview page about once a week (Geekbench's own site is behind a bot check, which this project never gets around). Every such result is labelled as matched automatically, and a phone with no chip recorded is left out of the chip-checked sources rather than guessed.</li>
              <li><strong>Samsung Malaysia service pages.</strong> Samsung Malaysia publishes no list of service programmes, so its screen-replacement and service-centre pages are read every 6 hours and checked for free, discount, promotion or extended-warranty wording, dates and any part of Malaysia they name; the site shows when a page last changed.</li>
              <li><strong>New phones and tablets join the hub by themselves.</strong> Every day the Malaysian sitemaps of OPPO, HONOR, Huawei, vivo, realme and OnePlus are read (only where the maker's robots.txt allows it). A specification page that appears there is read with the same fixed patterns used for the hand-reviewed records, and the model is added when the page is the maker's own, names the model in its title, matches its address, isn't a special edition of a model already in the hub, and gives the screen size, battery and the chip or cameras. Values outside plausible ranges, a battery given per cell, and a camera list where a high-resolution telephoto could be mistaken for the main camera are left out rather than guessed. New iPhones and iPads are found on Apple Support Malaysia, which lists every model with its own tech-specs page, and read from that page (Apple's own model name decides the screen size, as Apple names it). Samsung's specification pages are drawn in the browser and its specification service doesn't allow automated reading, so a new Galaxy phone or tablet is read from Samsung Newsroom Malaysia's own launch announcement: its specification table when it has one, otherwise its sentences, with the Malaysian launch price when the announcement states one (an announcement naming several models waits for a person). The same announcements fill in the Malaysian launch price of a Samsung model already in the hub that has none. Models several news sources are talking about are also looked up on Xiaomi's Malaysian site by their address. The announced date is the day the model first appeared on the maker's Malaysian site, or the date of Samsung's announcement, and the page says so. Sony's site doesn't allow automated readers, so its new models are still added by hand. Each device added this way is labelled "Added automatically", links to the maker's page for every value and is replaced as soon as a person adds a checked record. A number is kept only where the page says what it is, so a charging adapter's wattage, a touch sampling rate or a high-brightness-mode figure is never taken for the phone's own. Tested on 49 phones already in the hub, the automatic reading gave the same value as the checked record 427 times out of 429.</li>
              <li><strong>Trusted Reviews' test results join the charts by themselves.</strong> Trusted Reviews publishes a Test Data table in each phone review (Geekbench 6, the battery used by an hour of Netflix HDR playback, charging times). New reviews are read every day and all reviews about once a month, for phones with no hand-entered Trusted Reviews result, when the table names the phone exactly. Charging results measured without the maker's charger are left out, as by hand; Samsung models recorded with an Exynos chip are left out because Trusted Reviews tests UK models and doesn't name the chip; and a Geekbench result more than 25% from what the same chip scores in other phones waits for a person (the first run held three). Each review is labelled as read automatically.</li>
              <li><strong>Makers' spec pages re-read.</strong> About once a week, in turn, the makers' specification pages that devices cite are read again. A value the record doesn't have yet is filled from the page and marked "auto" in the specification table; a value the page now gives differently is never changed automatically, it is listed for a person to check.</li>
              <li><strong>Pictures.</strong> The automatic picture search now also reads the product page that goes with a specification page and the product data inside the page, and it runs every 3 days. It still takes only pictures served from the maker's own image server, never a banner or a picture filed under another model, and it waits as long as each site asks between pages. realme's pages offer lifestyle photos rather than the product, so realme pictures are only added by hand.</li>
              <li><strong>Auto-update labels.</strong> Every section that updates by itself carries a small “Auto” label saying how often; hover it for the source.</li>
            </ul>
