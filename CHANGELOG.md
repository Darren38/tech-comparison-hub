...
**Type a device name any way you like: search, Ask the hub and the AI**
- Names typed joined up ("s26ultra", "iphone17promax", "redminote14"), with extra spaces ("S26   ULTRA"), without the brand ("find x9 ultra", "mate x7") or in short form ("S26U", "ip17pm", "i17pm", "RN14 Pro+", "ZFold7", "fold7", "Tab S11U", "Find X9U", "P80 Ultra", "ROG 9", "s24+") now find the right model, in the search box, in Ask the hub and in the AI.
- The short forms are made from the device names themselves (Pro Max → PM, Ultra → U, Pro Plus → P+, iPhone → IP / i, Redmi Note → RN, Galaxy Z Fold → Fold, Galaxy Tab → Tab, Pura → P, ROG Phone → ROG, with or without the brand and "5G"), so every brand gets them, and so do models added later, including the automatic ones. When a short form fits more than one model, a model's own name comes first, then the short names already listed on a record ("17PM" is listed for the iPhone 17 Pro Max); otherwise ("Band 10": Huawei or HONOR) it is not guessed in a question, and the search box lists every model it fits.
- The search box also finds the models in a whole sentence: "should i buy s26u or 17pm" shows the Galaxy S26 Ultra and the iPhone 17 Pro Max.
- Fixed: the AI answered "is s26ultra worth upgrade from s24ultra" as if neither phone were listed and looked up "best phone". The models the site's own rules find in the question now always count, and "upgrade from", "switch", "replace" make it a comparison ("worth it" about one model gets a verdict).
- Fixed: a name typed joined up with its brand ("HONORX9ePro") could lose to a news headline in the search box; "Pixel Fold" showed the Pixel 10 Pro Fold first (all three share the series name); "watch ultra 2" showed the Galaxy Watch Ultra (2025) first (a short number no longer matches the start of a year); the Galaxy A07 5G, A17 5G and similar models with a separate 4G version were read as the 4G model in a question when the brand was typed in full.
- Everyday words are not read as models: "android 16 update" is not the Xiaomi "16U", "lighter than 175g" is not the Redmi 17 5G.
- The AI: a model it makes up from a name the site already found is no longer reported as "not in this database" ("rn14pro+ battery good?" also produced an "iPhone 14 Pro+"); "switch from … to …" is a comparison; a new check removes "too close to call" when the site's data shows one model ahead (78 vs 70). Tested with Qwen3.5 4B on "is s26ultra worth upgrade from s24ultra", "ip17pm or s26u for camera?", "rn14pro+ battery good?", "should i switch from iphone15 to pixel10pro" and "zfold7 vs matex7 which lighter": every model recognised, every answer checked against the data.
- Tested on every one of the 714 devices, typed nine ways (exact, lower case, upper case, joined up, spaced out, with and without the brand, short form, short form joined up): every full name is now found 100% of the time in search however it is typed (joined up was 97%), brand-less joined-up names 92% (was 49%), joined-up short forms 95% (was 34%); what is left is mostly ambiguous ("X8", "A7", "Band 10"). Ask the hub finds every full name however it is spaced, and 92% of short forms (was 80%).

**Data fix: Huawei Mate X7 screen**
- The Mate X7 was recorded with its cover screen's size (6.49 inches) next to its main screen's resolution, so its pixel density was worked out as 505 ppi. Huawei Malaysia's specification page gives the main screen as 8 inches, 2416 × 2210, 412 PPI, 1–120 Hz: corrected, and every other screen in the hub was checked the same way (size, resolution and pixel density agree for all 443 that have all three; the four round watch screens were checked by their width).
