...
**Devices added automatically (Version 20).** `live/auto/new_devices.json` is written by `tools/auto_devices.py`: `devices` (full device records in the same schema, with `dataStatus: "auto"` and `auto: {addedAt, url, site}`), `chipsets` (minimal records for chips named on those pages), `fills` (`{deviceId: {specPath: {value, url, checked}}}`, values missing from a record, read from the maker's page it cites), `changes` (values a maker's page now gives differently; listed for a person, never applied), `held` (new models that failed a check, with the reason) and the bookkeeping (`seen` spec pages per maker, `pending`, `runs`). The build adds these before validation; a record in `data/` with the same id always wins, and an automatic record that fails validation is left out so the build still publishes. `tools/devices_all.py` gives every tool the same list of devices.
