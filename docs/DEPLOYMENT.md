...
| Trusted Reviews test results (Geekbench, video drain, charging) | Once a day for new reviews, every review about once a month | `tools/auto_reviews.py`: the review's Test Data table, exact model name, phones with no hand-entered result; results far from the chip's usual score are held | The saved results stay |
...
| New phones and tablets (a spec page that newly appears on OPPO, HONOR, Huawei, vivo, realme or OnePlus Malaysia's sitemap; a new iPhone or iPad on Apple Support Malaysia; a Galaxy launch announcement on Samsung Newsroom Malaysia, which also fills missing Samsung Malaysian prices; Xiaomi pages for models in the news) | Once a day, same cache | `tools/auto_devices.py`: added only when the maker's page passes the checks, labelled "Added automatically"; otherwise held for a person | Nothing is added until the next day |
| Makers' spec pages cited by devices (about 120 a week, in turn) | About once a week | `tools/auto_devices.py`: missing values filled (marked "auto"); changed values only listed | The saved fills stay |
