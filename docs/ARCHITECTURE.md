...
│   ├── engine/              scoring, compare, search, names (how names are typed), intent, money (incl. live rates), live (headlines), evidencePack
...
- `engine/names.js` — how visitors type device names: `shortForms(name)` (Pro Max → PM, Ultra → U, Pro Plus → P+, iPhone → IP / i, Redmi Note → RN, Galaxy Z Fold → Fold, Galaxy Tab → Tab, ROG Phone → ROG, Pura → P, "5G" dropped), `typedForms()` (squashed form → the one model it means: own name, then a record's alias, then a made-up form; same-brand ties go to the base model) and `sharedForms()` (forms that fit several models). Used by `search.js` (whole names in any spacing, short forms, partly typed forms, models inside a sentence) and by `assistant.js` / `ai-agent.js` (models in a question), so both always agree and new models get the same forms.
